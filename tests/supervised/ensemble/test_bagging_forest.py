...
from rice_ml.Supervised_Learning.ensemble_methods.bagging_forest_class import BaggingForestClassifier
...
def test_bagging_forest(sample_data):
    X, y = sample_data
    model = BaggingForestClassifier(n_estimators=10, max_depth=3, random_state=42)
    model.fit(X, y)
    predictions = model.predict(X)
    assert np.array_equal(predictions, y), f"Expected {y}, but got {predictions}"

def test_bagging_forest_unseen_data(sample_data):
    X, y = sample_data
    model = BaggingForestClassifier(n_estimators=10, max_depth=3, random_state=42)
    model.fit(X, y)
...
def test_bagging_forest_invalid_input():
    model = BaggingForestClassifier(n_estimators=10, max_depth=3, random_state=42)
...
def test_bagging_forest_predict_before_fit():
    model = BaggingForestClassifier(n_estimators=10, max_depth=3, random_state=42)
