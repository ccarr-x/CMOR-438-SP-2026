...
def test_multilayer_perceptron(sample_data):
    X, y = sample_data
    model = MultiLayerPerceptron(hidden_layer_sizes=(5,), eta=0.1, epochs=10000, random_state=42)
    model.train(X, y)
...
def test_multilayer_perceptron_unseen_data(sample_data):
    X, y = sample_data
    model = MultiLayerPerceptron(hidden_layer_sizes=(5,), eta=0.1, epochs=1000, random_state=42)
    model.train(X, y)
...
def test_multilayer_perceptron_invalid_input():
    model = MultiLayerPerceptron(hidden_layer_sizes=(5,), eta=0.1, epochs=1000)
    with pytest.raises(ValueError):
        model.train([[0, 0], [0, 1]], [0])  # Mismatched number of samples and labels
    with pytest.raises(ValueError):
        model.train([[0, 0], [0, 1]], [0, 1, 1])  # Mismatched number of samples and labels
...
def test_multilayer_perceptron_learning_rate_effect(sample_data):
    X, y = sample_data
    model = MultiLayerPerceptron(hidden_layer_sizes=(5,), eta=1.0, epochs=1000, random_state=42)
    model.train(X, y)
    predictions = model.predict(X)
    assert np.array_equal(predictions, y), f"Expected {y}, but got {predictions}"


def test_multilayer_perceptron_zero_learning_rate():
    with pytest.raises(ValueError, match="eta must be positive"):
        MultiLayerPerceptron(hidden_layer_sizes=(5,), eta=0.0, epochs=1000)


def test_multilayer_perceptron_non_convergence(sample_data):
    X, y = sample_data
    model = MultiLayerPerceptron(hidden_layer_sizes=(5,), eta=10.0, epochs=100, random_state=42)
    model.train(X, y)
    predictions = model.predict(X)
...
def test_multilayer_perceptron_large_hidden_layer(sample_data):
    X, y = sample_data
    model = MultiLayerPerceptron(hidden_layer_sizes=(100,), eta=0.1, epochs=10000, random_state=42)
    model.train(X, y)
    predictions = model.predict(X)
    assert np.array_equal(predictions, y), f"Expected {y}, but got {predictions}"
...
def test_multilayer_perceptron_multiple_hidden_layers(sample_data):
    X, y = sample_data
    model = MultiLayerPerceptron(hidden_layer_sizes=(5, 5), eta=0.1, epochs=10000, random_state=42)
    model.train(X, y)
    predictions = model.predict(X)
    assert np.array_equal(predictions, y), f"Expected {y}, but got {predictions}"